...
@pytest.fixture
def fixture_tag_class_object(fixture_class_child_instance: Child):
    """Fixture class for testing tag system, object overrides class tags."""
...
def test_clone_tags():
    ...
    class TestClass(BaseObject):
        _tags: ClassVar[dict[str, Any]] = {"some_tag": True, "another_tag": 37}
...
def test_components_raises_error_base_class_is_not_class(
    ...
    msg = base_msg + "but found <class 'predictably_core.core._base.BaseObject'>"
    with pytest.raises(
        TypeError,
        match=msg,
    ):

...
Used to store information about predictably_core's global configuration.
"""
    ...
    def get_valid_param_or_default(
        ...
        if self.is_valid_param_value(value):
            ...
            expected_type_str = format_sequence_to_str(
                self.get_expected_type(), last_sep="or", exclude_type_text=True
            )
            msg += f"When setting global config values for `{self.name}`, the values "
            msg += f"should be of type {expected_type_str}.\n"
            if self.allowed_values is not None:
                values_str = format_sequence_to_str(
                    self.get_allowed_values(), last_sep="or", exclude_type_text=True
                )
